...
import { normalizarValorMonetario, coalesceValorMonetario } from "@/lib/valor-monetario";
...
export async function loadReciboViews(ids: string[]): Promise<ReciboView[]> {
  ...
  for (const it of items) {
    ...
    (byRec[it.recibo_id] ||= []).push({
      ...
      valor: coalesceValorMonetario(it.valor_snapshot),
      lancado_por: nome ?? "",
    });
  ...
  return ((recs ?? []) as Rec[])
    .map((r) => ({
      ...
      valor_total: normalizarValorMonetario(r.valor_total),
      ativo: r.ativo,
